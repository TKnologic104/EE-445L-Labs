<<<<<<< HEAD:Labs/Lab_3/Lab_3-ks38728_tk8435/main.c
/********* main.c **************
...
void InitBlock(void){
	...
	Timer1A_Init30sec();               // set up Timer0A for 30seconds
	InitBackground();
	InitScreen();
}
...
int main(void){
  ...
  SYSCTL_RCGCGPIO_R |= 0x20;            // activate port F
	...
	while(1){
		EnableInterrupts();
		if (alarmON == 1 && currentHour == alarmHour && currentMinute == alarmMinute){
			alarmON = 0;
			StartAlarm();
		}
		if (BtnPressed == NONE && CurrState == MAIN_SCREEN){ //BtnPrssed == None so that is continues to draw the main. when is isnt none is should continue to draw another screen
			...
			if (checkIdle()){ // this line says every 15mins the code will send you back to the main_screen
				...
				DrawMainScreenWithBkgnd(currentHour, currentMinute);
			}
		}
	}
}
...
>>>>>>> 72a0e10a274e2e923d69801230e0b6af9000521a:Labs/Lab_3-DEBOUNCE_FIXED_MAYBE/main.c
